...
export default function FacettenShowcase() {
    // zentrales Albumcover aus deinem echten Pfad
    const albumCover =
    ...
    // Halbkreis-Parameter
    const radius = 340; // wie weit die Covers vom Mittelpunkt weg sind
    const startAngle = -110; // linker Start
    const endAngle = 110; // rechter Endpunkt

    const step =
        songs.length > 1 ? (endAngle - startAngle) / (songs.length - 1) : 0;
